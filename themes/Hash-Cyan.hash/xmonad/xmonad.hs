...
myWorkspaces = fontawesome . (map xmobarEscape) $ [ ws1,ws2,ws3,ws4,ws5,ws6,ws7,ws8,ws9]
    where                                                                      
      fontawesome l = [ "<action=xdotool key super+" ++ show (n) ++ "><fn=1>" ++ ws ++ "</fn></action>" |
...
myStartupHook = do
  setWMName "xmonad"
  spawn     "pkill trayer ; trayer --edge top --align left --widthtype request --height 21 --alpha 0 --transparent true --monitor primary --width 3 --iconspacing 5 --tint 0x0d0d0d"
